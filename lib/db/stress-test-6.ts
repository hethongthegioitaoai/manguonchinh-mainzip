...
async function test2(ctx: Awaited<ReturnType<typeof setup>>) {
  ...
  // Reset army to known-good state before testing (Test 1 may have depleted it)
  const resetPow = calcPower(100, 85, 8.0, 90);
  await db.update(militaryForces).set({ totalSoldiers: 100, morale: 85, trainingLevel: 8.0, supplyLevel: 90, militaryPower: resetPow }).where(eq(militaryForces.id, armyA.id));
  await db.update(npcGovernments).set({ treasury: 2000 }).where(eq(npcGovernments.id, govA.id));

  // Drain treasury to 0 to force starvation
...
async function test5(ctx: Awaited<ReturnType<typeof setup>>) {
  ...
  const npcRows = await db.select({ id: npcCores.id }).from(npcCores).where(eq(npcCores.worldSlug, ctx.slug));
  const npcCount = npcRows.length;
  npcCount > 0 ? pass(`NPC count ổn định: ${npcCount} NPCs`) : fail("NPC count = 0");

  // Check DB has no negative-soldier anomalies
  const allArmies = await db.select({ id: militaryForces.id, soldiers: militaryForces.totalSoldiers }).from(militaryForces).where(inArray(militaryForces.governmentId, govIds));
  const negSoldiers = allArmies.filter(a => a.soldiers < 0).length;

...
> **Cập nhật lần cuối:** 21/06/2026 — PHASE 7.1B SNAPSHOT FALLBACK hoàn tất: extract `buildSnapshotData()` helper từ `saveWorldSnapshot()` trong `worldSimulation.ts`; endpoint `GET /api/simulation/snapshot/:worldSlug/:tick` không còn 404 — nếu không có persisted snapshot thì build synthetic từ live state, trả HTTP 200 với `syntheticSnapshot: true`; persisted snapshot trả `syntheticSnapshot: false`; Replay page hoạt động từ Tick 1. Không thay đổi frontend.

> **Cập nhật lần cuối:** 21/06/2026 — PHASE 7.1A DATABASE INDEX HARDENING hoàn tất: tạo migration `lib/db/migrations/0001_phase7_world_slug_indexes.sql`, cập nhật 3 schema files (npcFactions.ts/npcCore.ts/militaryForces.ts) thêm `index()` Drizzle definitions, apply 4 indexes mới: `npc_factions_world_slug_idx`, `npc_cores_world_slug_idx`, `military_forces_territory_id_idx`, `military_forces_government_id_idx`. Chú ý: `military_forces` không có cột `world_slug` trực tiếp — filter qua JOIN `territories(world_slug)`, nên thêm `territory_id` FK index thay vì world_slug; cost JOIN giảm từ 13.41 → 2.45 (82%). Seq Scan vẫn hiển thị trên bảng nhỏ (< 50 rows/slug) — PostgreSQL optimizer đúng: sẽ dùng index khi row count tăng. No business logic thay đổi. `pnpm --filter @workspace/db run push` → Changes applied.

> **Cập nhật lần cuối:** 21/06/2026 — PHASE 7 VALIDATION CHECKLIST hoàn tất: chạy toàn bộ 9 steps trên DB thật, sinh `docs/PHASE_7_VALIDATION_REPORT.md`. Kết quả: **85/85 checks PASS, Readiness Score 97%**. Step 1 DB audit: 10 tables kiểm tra, 0 orphan, 0 null anomaly; phát hiện 3 tables thiếu index world_slug (npc_factions/npc_cores/military_forces). Step 2 API: 12 public endpoints 200 OK, 10 auth-protected 401 đúng, empty-state graceful. Step 3 Events: 8 event types (territory_capture/collapse/recolonized/npc_migrate/army_move/arrived/siege/faction_leader_changed) ghi vào world_event_log + đọc được qua event-stream + xuất hiện trong history. Step 4 Snapshots: 6/6 ticks (50–300) PASS trên 3 worlds. Step 5 Political Map: 5 layers (territories/factions/armies/npcs/recentHistory) đầy đủ trên 3 worlds. Step 6 War: ownerFactionId đổi, population−15, security−20, territory_capture event + history đúng. Step 7 Collapse→Recolonization: active→ruins→active chain verified 2 lần, event chain đúng thứ tự. Step 8 Stress test 1000 ticks: 0 anomaly/0 NaN/0 negpop/0 runaway, economy oscillation [11.46–98.56], pop tăng 1013→2789. Step 9 Regression: 20/20 systems OK. Known issues: (1) slug mismatch khi seed, (2) snapshot 404 trước tick 50, (3) missing indexes 3 tables, (4) lastTickSent in-memory reset. Recommended fixes 4 items priority cao. Report: `docs/PHASE_7_VALIDATION_REPORT.md`.

> **Cập nhật lần cuối:** 21/06/2026 — PHASE 7 UNITY AUDIT hoàn tất: viết `docs/PHASE7_COMPLETION_REPORT.md` — kiểm toán toàn bộ 7 subsystem Unity (World Loader / Territory Renderer / Heatmaps / Army Renderer / Movement / Event Effects / Realtime Stream). Verdict: 5 PASS · 2 PARTIAL (Army Renderer + Movement do thiếu `DeltaEvent.cs` + `ApplyDeltaEvent()` chưa xác minh). Xác định 7 missing dependencies (TextMeshPro package, MiniJSON/Newtonsoft, DeltaEvent.cs, MapInputHandler, HeatmapLayer prefab child, FlashLayer, movementTick auto-call), 6 backend bottlenecks (6 sequential queries trong map-state, không ETag cache, in-memory cursor leak), 5 Unity bottlenecks (heatmap O(n)/frame, EventEffectManager ProcessQueue mỗi frame, ring buffer 800KB/world, MPB race condition, JsonUtility parse trên main thread). Unity optimization plan 12 bước + Phase 8 NPC Brain roadmap 6–8 ngày (NpcBrainClient / NpcBrainPanel / DialogueBubble / DialogueSessionManager / NpcOverlayRenderer / EmotionParticleController). Report: `docs/PHASE7_COMPLETION_REPORT.md`.

> **Cập nhật lần cuối:** 21/06/2026 — WEBSOCKET EVENT BUS (Phase 65A-65D) hoàn tất: (1) **Phase 65A — Canonical WorldEvent format**: `lib/unityWs.ts` refactor thêm `broadcastEvent(evt: WorldEvent)` với format chuẩn `{event, worldSlug, tick, ts, payload}`; legacy `broadcastUnity(LegacyUnityEvent)` giữ nguyên để backward compat; (2) **Phase 65B — Persistent Event Log**: bảng `world_event_log` (worldSlug/tick/event/payload jsonb/ts bigint + indexes); `eventBus.ts` với `emitEvent()` (DB write + WS broadcast) và `emitEventSync()` (fire-and-forget); `worldSimulation.ts` + `military.ts` đã fix dùng emitEvent/emitEventSync thay broadcastUnity cũ bị broken; 2 endpoints: `GET /api/unity/event-stream/:worldSlug` (replay với filter sinceTick/sinceTs/limit/event) + `/latest`; (3) **Phase 65C — Frontend Subscription Hook**: `useWorldEvents.ts` — WS lifecycle, auto-reconnect exponential backoff, catch-up replay on reconnect (fetch missed events), 100-event buffer, events/sec stats, `{events, latestEvent, connected, stats, clearEvents}`; (4) **Phase 65D — PoliticalMapPage Integration**: `useWorldEvents(worldSlug)` hook tích hợp, poll giảm 8000ms→30000ms, refetch on events (territory_capture/collapse/recolonized/army_move/arrived/siege/npc_migrate/faction_leader_changed/world_tick), WS status indicator trên toolbar (dot xanh animate-pulse khi connected, vàng khi off, show total event count); WS endpoint chuyển sang `/api/ws/unity` (dùng Vite proxy `/api` với ws:true); HMR path đặt `/__vite_hmr` để tránh WS conflict.

> **Cập nhật lần cuối:** 21/06/2026 — WORLD SEEDER + NPC LAYER (Phase 64 + WorldSeeder) hoàn tất: (1) **WorldSeederPage** tại `/world-seeder`: Full Auto Seed sequence (Defaults→Territories→WorldMap→NPCs→Military→Policies→N ticks), individual step buttons, progress bar, nhật ký thực thi realtime, link sang /political-map và /simulation-analytics; (2) **Phase 64 — NPC Layer trên Political Map**: map-state endpoint thêm `npcs[]` (id/name/occupation/territoryId/energy/hunger/happiness/currentGoal, limit 300), `PoliticalMapPage` thêm NPC cluster dot per territory (màu theo occupation: 💰Thương Nhân=#f59e0b / 🌾Nông Dân=#22c55e / ⚔Chiến Sĩ=#ef4444 / 📿Tu Sĩ=#8b5cf6 / 🔨Thợ=#f97316), cluster size theo count (6-10px), hover tooltip top-5 NPC với name/energy/hunger, toggle button "👥 NPC (N)" trên toolbar; occupation color fn + icon fn; npcsByTerritory useMemo.

> **Cập nhật lần cuối:** 21/06/2026 — ARMY MOVEMENT LAYER (Phase 63A-63E) hoàn tất: (1) **Phase 63A — Logical Movement Backend**: thêm 5 cột vào `military_forces` — `currentTerritoryId`, `targetTerritoryId`, `movementProgress` (0.0-1.0), `movementStatus` (idle/moving/arrived/sieging), `recentPositions` (jsonb trail [{x,y,tick}]); state machine IDLE→MOVING→ARRIVED→IDLE/SIEGING; 3 API routes mới: `POST /military/move-order/:worldSlug` (phát lệnh di chuyển), `POST /military/movement-tick/:worldSlug` (tiến trình, progressDelta, auto-detect sieging khi địch khác phe), `POST /military/movement-reset/:worldSlug` (reset SIEGING→IDLE); Unity broadcast `army_movement_tick`; (2) **Phase 63B — Army Layer nâng cấp**: hover tooltip ⚔ Army Marker hiển thị đầy đủ tên/quân số/sức mạnh/tinh thần/tiếp tế/status%; (3) **Phase 63C — Movement Animation**: `armySvgPos()` dùng `lerp(sx(fromX), sx(toX), progress)` — army dot di chuyển mượt giữa 2 node theo movementProgress; (4) **Phase 63D — Trails**: render 8 trail dots từ `recentPositions` với opacity fade 10%→80%, radius lớn dần, màu cam #f97316; đường dashed cam từ origin đến target; (5) **Phase 63E — Siege Visualization**: blinking dashed red ring (700ms interval) trên territory bị bao vây, label "🔴 BỊ BAO VÂY" nhấp nháy; (6) map-state endpoint cập nhật trả đầy đủ movement data cho frontend.

> **Cập nhật lần cuối trước:** 21/06/2026 — POLITICAL MAP 2D (Phase 56) hoàn tất: (1) `GET /api/unity/map-state/:worldSlug` — endpoint mới không cần auth: territories (x/y/status/pop/prosperity/security/ownerFactionId), factions (influence/treasury), armies (soldiers/power/morale/supply qua gov→territory join), recentHistory (20 sự kiện gần nhất); (2) `PoliticalMapPage.tsx` tại `/political-map`: SVG 900×560 territory nodes tô màu theo phe phái (palette 10 màu deterministic từ factionId), kích thước node theo population, vòng ring prosperity heatmap, thanh security 3 màu (green/yellow/red), army ⚔ marker, grid nền cyber, tooltip hover, click để xem chi tiết territory + faction + army; toggles Thịnh Vượng / Quân Đội / LIVE; world selector 3 thế giới; Timeline Slider (tick 1→maxTick, sự kiện theo tick với icon/màu); panel phải: Territory Detail, Faction List (click highlight), Stats Summary (6 card), Recent History (20 events); (3) Dashboard button "BẢN ĐỒ CHÍNH TRỊ 2D" NEW; (4) route `/political-map` wire vào App.tsx. Endpoint history + timeline đổi sang public (no auth) để map renderer dùng được.

> **Cập nhật lần cuối trước:** 21/06/2026 — WAR v1 + WORLD HISTORY + RECOLONIZATION (Phase 55) hoàn tất: (1) `POST /api/military/attack/:worldSlug` — War v1 route: load attacker/defender gov+army+faction, combat 30 ticks với defender terrain bonus +20%, ownerFactionId transfer khi thắng, population/security cập nhật, territory_logs + npc_government_logs + military_memories, world_history entry, Unity broadcast `territory_captured`; (2) Bảng `world_history` — tick/eventType/title/description/actors(json); `GET /api/simulation/history/:worldSlug` (filter eventType, stats); `GET /api/simulation/history/:worldSlug/timeline` (Unity-ready compact); (3) Territory Collapse trong world tick: pop<10 + sec<15 → status="ruins" + world_history "collapse" + Unity broadcast; (4) Recolonization trong world tick: ruins + nearby overcrowded (pop>200, dx/dy<60) + 15% chance → status="active", settlers tách từ source territory, world_history "recolonization"; (5) Stress Test #7: 35 PASS / 0 FAIL / 0 WARN — 1000 ticks sạch, War chiếm lãnh thổ trong 7 ticks, ownerFactionId đổi đúng, 31 refugees, collapse đúng, recolonization 5 settlers, history 3 event types đúng, toàn bộ DB integrity.

> **Cập nhật lần cuối trước:** 21/06/2026 — SIMULATION HARDENING (Phase 54) hoàn tất: (1) Army Supply/Morale → Territory Security chain: khi army thiếu tiếp tế (supply<20 → security−rand(1,3), morale<30 → security−rand(1,2)); wiring vào supply route + stress test; (2) Recruitment Gate 3 điều kiện: treasury>50 AND territory.prosperity>30 AND territory.population>20; prosperityMult 0.4×/0.7×/1.0×; (3) Stress Test #6 nâng lên 500 ticks: 26 PASS / 0 FAIL / 1 WARN — economy oscillation [13.4,84.6], treasury không runaway, power không âm, population không âm, chain Food→Army→Security HOẠT ĐỘNG (security 60→41 sau 10 tick đói); endpoint `POST /api/simulation/stress-test/:worldSlug` (max 500 ticks, anomaly detection, JSON report). Simulation loop Food→Army Logistics→Territory Security→NPC Migration→Collapse KHÉP KÍN.

> **Cập nhật lần cuối trước:** 18/06/2026 — BATTLE SYSTEM UNITY BROADCAST (Phase 53.6) hoàn tất: `broadcastUnity()` wire vào `POST /api/battle/finish` — sau mỗi battle kết thúc emit event `battle` với payload đầy đủ (battleId/winner/loser/winnerName/loserName/expGained/goldReward/territoryChanged/timestamp); nếu player tham chiến bổ sung playerId/playerWon/levelUp; nếu lãnh thổ đổi chủ bổ sung territoryId/territoryName/captured; broadcast fail → log warning + không throw; `BattleEvent` interface tách riêng trong `unityWs.ts`; `GET /api/unity/ws-info` cập nhật schema đầy đủ với 2 ví dụ (player vs NPC và territory war). Cross-world broadcast sẵn sàng cho territory battle route khi được build.

> **Cập nhật lần cuối trước:** 18/06/2026 — UNITY BROADCAST WIRING (Phase 53.5) hoàn tất: `broadcastUnity()` được wire vào 4 route thực tế — `npcElections.ts` (broadcast `election` event khi resolve bầu cử, kèm territory/winner/electionType); `npcPopulation.ts` (broadcast `birth` event sau mỗi NPC sinh ra, kèm npcId/name/parentName); `npcDiplomacy.ts` (broadcast `diplomacy` event sau POST /action, resolve worldSlug từ territory của govA); `worldWar.ts` (broadcast `war_start` cho cả 2 thế giới khi tuyên chiến, broadcast `war_end` cho cả 2 thế giới khi settleExpiredWars kết thúc chiến tranh). Unity client giờ nhận realtime events khi chúng thực sự xảy ra, không chỉ khi poll.
